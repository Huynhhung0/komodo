...
UniValue setpubkey(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    ...
    if ( NOTARY_PUBKEY33[0] == 0 ) 
    {
        if (strlen(params[0].get_str().c_str()) == 66) 
        {
            ...
            } else result.push_back(Pair("error", "pubkey entered is invalid."));
        } else result.push_back(Pair("error", "pubkey is wrong length, must be 66 char hex string."));
    } 
    else result.push_back(Pair("error", "Can only set pubkey once, to change it you need to restart your daemon."));
    
    // TODO: this is for disablecb in Knomp, should move to a new RPC, so this RPC works the same as the rest.
    if ( NOTARY_PUBKEY33[0] != 0 )
    {
        pubkey2addr((char *)Raddress,(uint8_t *)NOTARY_PUBKEY33);
        result.push_back(Pair("address", Raddress));
