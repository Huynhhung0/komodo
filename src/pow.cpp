    ...
    if ( height > 34000 && ASSETCHAINS_SYMBOL[0] == 0 ) // 0 -> non-special notary
    {
        ...
        if ( notaryid >= 0 )
        {
            ...
            if ( (flag != 0 || special2 > 0) && special2 != -2 )
            {
                ...
                if ( height > nDecemberHardforkHeight && !komodo_checkopret((const CBlock*)pblock, merkleroot) ) // December 2019 hardfork
                {
                    fprintf(stderr, "failed or missing expected.%s != %s\n", komodo_makeopret((const CBlock*)pblock, false).ToString().c_str(), merkleroot.ToString().c_str());
                    return false;
                } */
            }
        }
